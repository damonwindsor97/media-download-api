...
router.route('/getTitle').post(async (req, res) => {
    ...
    try {
        ...
        const info = await ytdl.getInfo(videoUrl, { agents });
...
router.route('/downloadMp4').post(async (req, res) => {
    try {
        ...
        ytdl(videoUrl, { quality: 'highestaudio' }, { agents }).pipe(ffmpegProcess.stdio[4]);
        ytdl(videoUrl, { quality: 'highestvideo' }, { agents }).pipe(ffmpegProcess.stdio[5]);
...
router.route('/downloadMp3').post(async (req, res) => {    
    try {
        ...
        ytdl(videoUrl, { format: mp4Format }, { agents }).pipe(audioWriteStream);
